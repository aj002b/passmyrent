        ...
        <section className="form-card space-y-4 p-4 sm:p-5">
          ...
          >
            {Array.from({ length: count }, (_, index) => (
              <div key={index} className="rounded-2xl border border-[#D7E5EA] bg-white p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.75)]">
      ...
      result={
        <>
          <ResultCard title={title} description={description} tone={tone} badgeLabel={hasResult ? "Estimated split" : "Add details"}>
            {hasResult ? (
              <div className="overflow-x-auto rounded-2xl border border-[#D7E5EA] bg-white/82 shadow-[0_8px_18px_rgba(11,47,53,0.035)]">
                <table className="w-full min-w-[420px] text-left text-sm">
                  <thead className="border-b border-[#D7E5EA] text-[#5D6D75]">
                    <tr>
                  ...
                  <tbody>
                    {rows.map((row) => (
                      <tr key={row.name} className="border-b border-[#D7E5EA] last:border-0">
                        <td className="px-4 py-3 font-bold text-[#0B2F35]">{row.name}</td>
